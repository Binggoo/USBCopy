...
DWORD CSerialPort::ReadBytes( BYTE *byBuffer, DWORD dwBufferLength, DWORD dwWaitTime /*= 20*/ )
{
	if(!IsOpen())
	{
		return 0;
	...
	if(!::ReadFile(m_hCommHandle, byBuffer, dwBufferLength, &uReadLength, &m_ReadOverlapped)) //2000 下 ReadFile 始终返回 True
	{
		if(::GetLastError() == ERROR_IO_PENDING) // 结束异步I/O
		{
			WaitForSingleObject(m_ReadOverlapped.hEvent, dwWaitTime); //等待20ms
...
DWORD CSerialPort::WriteBytes( BYTE *byBuffer, DWORD dwBufferLength)
{
	if(!IsOpen())
	{
		return 0;
	...
	if(!::WriteFile(m_hCommHandle, byBuffer, dwBufferLength, &uWriteLength, &m_WriteOverlapped))
	{
		if(::GetLastError() != ERROR_IO_PENDING)
		{
			uWriteLength = 0;
...
bool CSerialPort::SetTimeOut( DWORD dwReadTimeOut,DWORD dwWriteTimeOut )
{
	//设置超时时间
	...
	m_CO.ReadTotalTimeoutConstant = dwReadTimeOut;
	m_CO.WriteTotalTimeoutMultiplier = 0;
	m_CO.WriteTotalTimeoutConstant = dwWriteTimeOut;
